...
public final class Profile implements Parcelable {
	...
	/**
	 * Setter for the profile that is currently logged in to the application. If
	 * the access token is invalidated, the current profile will not be updated.
	 * It's only updated when there is an explicit logout, login or when
	 * permissions change via the {@link com.facebook.login.LoginManager}.
	 * 
	 * @param profile
		...
		Utility.getGraphMeRequestWithCacheAsync(accessToken.getToken(),
				new Utility.GraphMeRequestWithCacheCallback() {
					...
					public void onSuccess(JSONObject userInfo) {
						...
						Profile profile = new Profile(id, userInfo
								.optString("first_name"), userInfo
								.optString("middle_name"), userInfo
								.optString("last_name"), userInfo
								.optString("name"), link != null ? Uri
								.parse(link) : null);
	 ...
	 *
	 * @param width
	 *            The desired width for the profile picture.
	 * @param height
