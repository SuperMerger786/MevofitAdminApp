...
/**
 * This class represents an immutable access token for using Facebook APIs. It
 * also includes associated metadata such as expiration date and permissions.
 * <p/>
...
public final class AccessToken implements Parcelable {
	 ...
	 * access token obtained prior to integration with the Facebook SDK). Note
	 * that the caller is asserting that all parameters provided are correct
	 * with respect to the access token string; no validation is done to verify
	 * they are correct.
	 *
	 * @param accessToken
	 ...
	 *            the permissions that were declined when the token was
	 *            obtained; may be null if permission set is unknown
	 ...
	 */
	public AccessToken(final String accessToken, final String applicationId,
			final String userId,
		...
		this.expires = expirationTime != null ? expirationTime
				: DEFAULT_EXPIRATION_TIME;
		this.permissions = Collections
				.unmodifiableSet(permissions != null ? new HashSet<String>(
						permissions) : new HashSet<String>());
		this.declinedPermissions = Collections
				.unmodifiableSet(declinedPermissions != null ? new HashSet<String>(
						declinedPermissions) : new HashSet<String>());
		this.token = accessToken;
		this.source = accessTokenSource != null ? accessTokenSource
				: DEFAULT_ACCESS_TOKEN_SOURCE;
		this.lastRefresh = lastRefreshTime != null ? lastRefreshTime
	...
	/**
	 * Updates the current access token with up to date permissions, and extends
	 * the expiration date, if extension is possible.
	 *
	 ...
	 * the most up-to-date list of permissions is maintained by Facebook, so
	 * this list may be outdated if permissions have been added or removed since
	 * the time the AccessToken object was created. For more information on
	 * permissions, see
	 ...
	 * by Facebook, so this list may be outdated if permissions have been
	 * granted or declined since the last time an AccessToken object was
	 * created.
	 *
	...
	public interface AccessTokenCreationCallback {
		 ...
		 *
		 * @param token
		 *            the access token created from the native link intent.
		 */
	...
	public static void createFromNativeLinkingIntent(Intent intent,
		...
		if (intent.getExtras() == null) {
			accessTokenCallback.onError(new FacebookException(
					"No extras found on intent"));
			return;
		}
		...
		if (accessToken == null || accessToken.isEmpty()) {
			accessTokenCallback.onError(new FacebookException(
					"No access token found on intent"));
			return;
		}

		String userId = extras.getString(USER_ID_KEY);
		// Old versions of facebook for android don't provide the UserId. Obtain
		// the id if missing
	...
	public boolean equals(Object other) {
		...
		return expires.equals(o.expires)
				...
				&& (applicationId == null ? o.applicationId == null
	...
	static AccessToken createFromRefresh(AccessToken current, Bundle bundle) {
		...
		if (current.source != AccessTokenSource.FACEBOOK_APPLICATION_WEB
			...
			throw new FacebookException("Invalid token source: "
					+ current.source);
		}

		Date expires = Utility.getBundleLongAsDate(bundle, EXPIRES_IN_KEY,
				new Date(0));
		...
		return new AccessToken(token, current.applicationId,
				current.getUserId(), current.getPermissions(),
				current.getDeclinedPermissions(), current.source, expires,
				new Date());
	}
	...
	static AccessToken createFromLegacyCache(Bundle bundle) {
		...
		String userId;
		JSONObject userInfo = Utility
				.awaitGetGraphMeRequestWithCache(tokenString);
		try {
			userId = userInfo.getString("id");
		} catch (JSONException ex) {
			// This code is only used by AccessTokenCache. If we for any reason
			// fail to get the
			// user id just return null.
			return null;
		}

		return new AccessToken(tokenString, applicationId, userId, permissions,
				declinedPermissions, LegacyTokenHelper.getSource(bundle),
				LegacyTokenHelper.getDate(bundle,
	...
	static AccessToken createFromJSONObject(JSONObject jsonObject)
		...
		JSONArray declinedPermissionsArray = jsonObject
				.getJSONArray(DECLINED_PERMISSIONS_KEY);
		Date lastRefresh = new Date(jsonObject.getLong(LAST_REFRESH_KEY));
		AccessTokenSource source = AccessTokenSource.valueOf(jsonObject
	...
	private static AccessToken createFromBundle(
		...
		return new AccessToken(token, applicationId, userId,
				requestedPermissions, null, source, expires, new Date());
	}
	...
	AccessToken(Parcel parcel) {
		...
		this.permissions = Collections.unmodifiableSet(new HashSet<String>(
				permissionsList));
		permissionsList.clear();
		parcel.readStringList(permissionsList);
		this.declinedPermissions = Collections
				.unmodifiableSet(new HashSet<String>(permissionsList));
