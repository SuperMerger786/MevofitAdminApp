...
public class FacebookDialogFragment extends DialogFragment {
	...
	/**
	 * Setter for dialog. The dialog should be set before the show method is
	 * called.
	 * 
	 * @param dialog
	 *            The dialog that is wrapped.
	 */
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (this.dialog == null) {
			...
			boolean isWebFallback = params.getBoolean(
					NativeProtocol.WEB_DIALOG_IS_FALLBACK, false);
			WebDialog webDialog;
			if (!isWebFallback) {
				String actionName = params
						.getString(NativeProtocol.WEB_DIALOG_ACTION);
				Bundle webParams = params
						.getBundle(NativeProtocol.WEB_DIALOG_PARAMS);
				...
				webDialog = new WebDialog.Builder(activity, actionName,
						webParams).setOnCompleteListener(
				...
				String redirectUrl = String.format("fb%s://bridge/",
						FacebookSdk.getApplicationId());
				webDialog = new FacebookWebFallbackDialog(activity, url,
						redirectUrl);
				webDialog
						.setOnCompleteListener(new WebDialog.OnCompleteListener() {
							...
							public void onComplete(Bundle values,
									FacebookException error) {
								// Error data is nested in the values since this
								// is in the form of a
	...
	private void onCompleteWebDialog(Bundle values, FacebookException error) {
		...
		Intent resultIntent = NativeProtocol.createProtocolResultIntent(
				fragmentActivity.getIntent(), values, error);

		int resultCode = error == null ? Activity.RESULT_OK
