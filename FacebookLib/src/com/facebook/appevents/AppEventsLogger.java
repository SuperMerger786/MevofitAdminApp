 ...
 * FacebookSdk.addLoggingBehavior(LoggingBehavior.APP_EVENTS);</li>
 * </ul>
 * </p>
 * <p>
...
public class AppEventsLogger {
	...
	public enum FlushBehavior {
		/**
		 * Flush automatically: periodically (every 15 seconds or after every
		 * 100 events), and always at app reactivation. This is the default
		 * value.
		 */
		AUTO,
	...
	private static class AccessTokenAppIdPair implements Serializable {
		...
		AccessTokenAppIdPair(String accessTokenString, String applicationId) {
			this.accessTokenString = Utility.isNullOrEmpty(accessTokenString) ? null
					: accessTokenString;
		...
		@Override
		public int hashCode() {
			return (accessTokenString == null ? 0 : accessTokenString
					.hashCode())
		...
		public boolean equals(Object o) {
			...
			return Utility.areObjectsEqual(p.accessTokenString,
					accessTokenString)
	...
	public static void activateApp(Context context, String applicationId) {
		...
		if ((context instanceof Activity)) {
			...
			// If context is not an Activity, we cannot get intent nor calling
			// activity.
		...
		// activateApp supersedes publishInstall in the public API, so we need
		// to explicitly invoke
		// it, since the server can't reliably infer install state for all
		// conditions of an app
		// activate.
		FacebookSdk.publishInstallAsync(context, applicationId);

		final AppEventsLogger logger = new AppEventsLogger(context,
				applicationId, null);
	...
	/**
	 * Notifies the events system that the app has been deactivated (put in the
	 * background) and tracks the application session information. Should be
	 * called whenever your app becomes inactive, typically in the onPause()
	 * method of each long-running Activity of your app.
	 *
	 * Use this method if your application ID is stored in application metadata,
	 * otherwise see
	 ...
	 *            Used to access the attributionId for non-authenticated users.
	 * @param applicationId
	 *            The specific applicationId to track session information for.
	 */
	...
	private void logAppSessionResumeEvent(long eventTime,
		...
		PersistedAppSessionInfo.onResume(applicationContext, accessTokenAppId,
				this, eventTime, sourceApplicationInfo);
	}

	private void logAppSessionSuspendEvent(long eventTime) {
		PersistedAppSessionInfo.onSuspend(applicationContext, accessTokenAppId,
				this, eventTime);
	}
	 ...
	 *            Used to access the attributionId for non-authenticated users.
	 * @param accessToken
	 *            Access token to use for logging events. If null, the active
	 *            access token will be used, if any; if not the logging will
	 *            happen against the default app ID specified in the package
	 *            metadata.
	 */
	public static AppEventsLogger newLogger(Context context,
	 ...
	 */
	public static AppEventsLogger newLogger(Context context,
			String applicationId, AccessToken accessToken) {
	 ...
	 *            hyphens, but must not have a space or hyphen as the first
	 ...
	 *            with each distinct eventName. It's advisable to limit the
	 *            number of unique values provided for each parameter in the
	 *            thousands. As an example, don't attempt to provide a unique
	 *            parameter value for each unique user in your app. You won't
	 *            get meaningful aggregate reporting on so many parameter
	 ...
	 *            This should have no more than 10 entries, and keys should be
	 *            mostly consistent from one purchase event to the next.
	 */
	public void logPurchase(BigDecimal purchaseAmount, Currency currency,
			Bundle parameters) {
	...
	private AppEventsLogger(Context context, String applicationId,
		...
		// If we have a session and the appId passed is null or matches the
		// session's app ID:
		if (accessToken != null
				&& (applicationId == null || applicationId.equals(accessToken
						.getApplicationId()))) {
	...
	private static void initializeTimersIfNeeded() {
		...
		backgroundExecutor.scheduleAtFixedRate(flushRunnable, 0,
				FLUSH_PERIOD_IN_SECONDS, TimeUnit.SECONDS);
		...
		backgroundExecutor.scheduleAtFixedRate(attributionRecheckRunnable, 0,
	...
	private void logEvent(String eventName, Double valueToSum,
		...
		AppEvent event = new AppEvent(this.context, eventName, valueToSum,
				parameters, isImplicitlyLogged);
	...
	// Creates a new SessionEventsState if not already in the map_run.
	private static SessionEventsState getSessionEventsState(Context context,
			AccessTokenAppIdPair accessTokenAppId) {
		// Do this work outside of the lock to prevent deadlocks in
		// implementation of
		// AdvertisingIdClient.getAdvertisingIdInfo, because that implementation
		// blocks waiting on
		...
		if (state == null) {
			// Retrieve attributionId, but we will only send it if attribution
	...
	private static FlushStatistics buildAndExecuteRequests(FlushReason reason,
		...
		boolean limitEventUsage = FacebookSdk
		...
		for (AccessTokenAppIdPair accessTokenAppId : keysToFlush) {
			...
			GraphRequest request = buildRequestForSession(accessTokenAppId,
	...
	private static GraphRequest buildRequestForSession(
		...
		requestParameters.putString("access_token",
				accessTokenAppId.getAccessTokenString());
		...
		int numEvents = sessionEventsState.populateRequest(postRequest,
				fetchedAppSettings.supportsImplicitLogging(), limitEventUsage);
	...
	private static void handleResponse(AccessTokenAppIdPair accessTokenAppId,
			GraphRequest request, GraphResponse response,
		...
		if (flushResult == FlushResult.NO_CONNECTIVITY) {
			...
			// since in principle we could call it once for all failed requests,
			// but the impact is
			// likely to be minimal. We don't call this for other server errors,
			// because if an event
			// failed because it was malformed, etc., continually retrying it
	...
	private static void setSourceApplication(Activity activity) {
		...
		if (callingApplication != null) {
			String callingApplicationPackage = callingApplication
					.getPackageName();
		...
		// Tap icon to open an app will still get the old intent if the activity
		// was opened by an
		// intent before. Introduce an extra field in the intent to force clear
		// the
		// sourceApplication.
		Intent openIntent = activity.getIntent();
		if (openIntent == null
				|| openIntent.getBooleanExtra(
						SOURCE_APPLICATION_HAS_BEEN_SET_BY_THIS_INTENT, false)) {
			resetSourceApplication();
			return;
		}
		...
		String applinkReferrerPackage = applinkReferrerData
				.getString("package");
		sourceApplication = applinkReferrerPackage;

		// Mark this intent has been used to avoid use this intent again and
		// again.
		openIntent.putExtra(SOURCE_APPLICATION_HAS_BEEN_SET_BY_THIS_INTENT,
				true);

		return;
	}

	static void setSourceApplication(String applicationPackage,
			boolean openByAppLink) {
	...
	/**
	 * Each app/device pair gets an GUID that is sent back with App Events and
	 * persisted with this app/device pair.
	 * 
	 * @param context
			...
			synchronized (staticLock) {
				if (anonymousAppDeviceGUID == null) {
					...
					SharedPreferences preferences = context
							.getSharedPreferences(APP_EVENT_PREFERENCES,
									Context.MODE_PRIVATE);
					anonymousAppDeviceGUID = preferences.getString(
							"anonymousAppDeviceGUID", null);
					if (anonymousAppDeviceGUID == null) {
						// Arbitrarily prepend XZ to distinguish from device
						// supplied identifiers.
						anonymousAppDeviceGUID = "XZ"
								+ UUID.randomUUID().toString();

						context.getSharedPreferences(APP_EVENT_PREFERENCES,
								Context.MODE_PRIVATE)
								.edit()
								.putString("anonymousAppDeviceGUID",
										anonymousAppDeviceGUID).apply();
					}
				}
			}
		}
	...
	static class SessionEventsState {
		...
		public SessionEventsState(AttributionIdentifiers identifiers,
				String packageName, String anonymousGUID) {
		...
		// coming in from
		// different AppEventsLoggers on different threads pointing at the same
		// session.
		...
		public int populateRequest(GraphRequest request,
				boolean includeImplicitEvents, boolean limitEventUsage) {
		...
		public synchronized List<AppEvent> getEventsToPersist() {
			...
			// in-flight. This means if
			// an in-flight request fails, those requests will not be persisted
			// and thus might be
		...
		public synchronized void accumulatePersistedEvents(List<AppEvent> events) {
			...
			// accumulated them once and
			// persisted them. But they will count against the buffer size when
			// further events are
			// accumulated.
			accumulatedEvents.addAll(events);
		}

		private void populateRequest(GraphRequest request, int numSkipped,
				JSONArray events, boolean limitEventUsage) {
			JSONObject publishParams = null;
			try {
				publishParams = AppEventsLoggerUtility
						.getJSONObjectForGraphAPICall(
								AppEventsLoggerUtility.GraphAPIActivityType.CUSTOM_APP_EVENTS,
								attributionIdentifiers, anonymousAppDeviceGUID,
								limitEventUsage, applicationContext);
	...
	static class AppEvent implements Serializable {
		...
		public AppEvent(Context context, String eventName, Double valueToSum,
				Bundle parameters, boolean isImplicitlyLogged) {
			try {
		...
		private void validateIdentifier(String identifier)
			...
			if (identifier == null || identifier.length() == 0
				...
				throw new FacebookException(String.format(Locale.ROOT,
	...
	static class PersistedAppSessionInfo {
		...
		private static final Runnable appSessionInfoFlushRunnable = new Runnable() {
			...
			public void run() {
				PersistedAppSessionInfo
						.saveAppSessionInformation(applicationContext);
			}
		};
			...
			synchronized (staticLock) {
				if (!isLoaded) {
					try {
						...
						appSessionInfoMap = (HashMap<AccessTokenAppIdPair, FacebookTimeSpentData>) ois
								.readObject();
						Logger.log(LoggingBehavior.APP_EVENTS, "AppEvents",
						...
						// Regardless of the outcome of the load, the session
						// information cache
						// is always deleted. Therefore, always treat the
		...
		static void onResume(Context context,
				AccessTokenAppIdPair accessTokenAppId, AppEventsLogger logger,
				long eventTime, String sourceApplicationInfo) {
			synchronized (staticLock) {
				FacebookTimeSpentData timeSpentData = getTimeSpentData(context,
						accessTokenAppId);
				timeSpentData
						.onResume(logger, eventTime, sourceApplicationInfo);
				onTimeSpentDataUpdate();
			}
		}

		static void onSuspend(Context context,
				AccessTokenAppIdPair accessTokenAppId, AppEventsLogger logger,
				long eventTime) {
			synchronized (staticLock) {
				FacebookTimeSpentData timeSpentData = getTimeSpentData(context,
		...
		private static FacebookTimeSpentData getTimeSpentData(Context context,
				AccessTokenAppIdPair accessTokenAppId) {
		...
		private static void onTimeSpentDataUpdate() {
			if (!hasChanges) {
				hasChanges = true;
				backgroundExecutor.schedule(appSessionInfoFlushRunnable,
						FLUSH_APP_SESSION_INFO_IN_SECONDS, TimeUnit.SECONDS);
			}
		}
	}
	...
	static class PersistedEvents {
		...
		public static void persistEvents(Context context,
			...
			synchronized (staticLock) {
				...
				// added a particular
				// event to SessionEventsState; when a particular Context is
				// being destroyed, we'll
				// persist all accumulated events. More sophisticated tracking
				// could be done to try
				// to reduce unnecessary persisting of events, but the overall
				// number of events is
		...
		private void readAndClearStore() {
			...
			try {
				...
				// means we'd prefer to
				// lose some events in the case of exception rather than
